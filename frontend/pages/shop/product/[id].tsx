...
function ProductDetail({ id }: any) {
  ...
  const { data: moreItemList } = useGetFlowers({ params: { limit: 4, page: 1 } });
  ...
  const renderImageGroup = useMemo(() => {
    return (
      <>
        ...
        <AppCarousel list={listImage} numberItemPerView={smallScreen ? 3 : 5} onChange={setMainImg} />
      </>
    );
  }, [mainImg, listImage]);
  ...
  return (
    <main className='container product-detail'>
        ...
        <Col span={24} lg={14}>
          ...
          <Row className='product-detail__topic-list' gutter={[16, 16]}>
            {listTopics?.map(({ name }: { name: string }) => {
              return <Col className='product-detail__topic-list__item'>{name}</Col>;
            })}
          </Row>
        </Col>
        <Col span={24} lg={10}>
          ...
          <div className='product-detail__desc-group'>
            ...
            <div className='product-detail__desc-group__content'>{flower?.description}</div>
          </div>
          <button className='product-detail__send-inquiry-btn' onClick={() => setOpenModalSendInquiry(true)}>
            Send Inquiry
          </button>
        </Col>
      </Row>
      <div className='product-detail__more-item'>
        <p className='product-detail__more-item__title'>MORE ITEMS TO CONSIDER</p>
          ...
          {moreItemList?.items?.map(({ listImage, name, price }: any) => {
            return (
              <Col span={24} sm={12} lg={6}>
                ...
                <p className='product-detail__more-item__item-price'>From ${price}</p>
              </Col>
            );
          })}
        </Row>
      ...
      <Modal
        ...
        width={400}
      >
        <div className='product-detail__send-inquiry-modal__title'>Send an Inquiry</div>
        <p className='product-detail__send-inquiry-modal__desc'>
          Just leave your contact below. We will get in touch with you shortly!
        </p>
        <Form form={form} layout='vertical' onFinish={() => {}} onFinishFailed={() => {}} autoComplete='off'>
          <Form.Item name='name' label='Your name' rules={[{ required: true }, { type: 'string', min: 6 }]}>
            <Input placeholder='' />
          </Form.Item>
          <Form.Item name='mail' label='Email address' rules={[{ required: true }, { type: 'email' }]}>
            <Input placeholder='' />
          </Form.Item>
          <Form.Item name='phone' label='Phone Number' rules={[{ required: true }, { type: 'number', min: 9 }]}>
            <Input placeholder='' />
          </Form.Item>
        </Form>
        <button
