...
export default function WebInfo() {
  ...
  const setStateFromApi = () => {
    const { title, email, facebookLink, mobilePhone, zaloLink, address } = webInfo?.data || {};
    setTitle(title || '');
    setEmail(email || '');
  ...
  const handleClickSave = async () => {
    await mutateAsync({
      params: { title, zaloLink, facebookLink, email, address, mobilePhone: phoneNumber },
      id: webInfo?.data?.id,
    });
  };
  ...
  return (
    <div className='web-info'>
      ...
      <div className='web-info__form'>
        ...
        <TextInputWithLabel label='Link Zalo' value={zaloLink} onChange={setZaloLink} disabled={!isEdit} />
        <TextInputWithLabel label='Link Facebook' value={facebookLink} onChange={setFacebookLink} disabled={!isEdit} />
        <TextInputWithLabel label='Số điện thoại' value={phoneNumber} onChange={setPhoneNumber} disabled={!isEdit} />
        <TextInputWithLabel label='Địa chỉ' value={address} onChange={setAddress} disabled={!isEdit} />
        <TextInputWithLabel label='Email' value={email} onChange={setEmail} disabled={!isEdit} />
      </div>
      <Row gutter={12}>
        {!isEdit ? (
          <Col>
          ...
          <>
            <Col>
              <AppButton className='web-info__button' text='Hủy' onClick={handleClickCancel} />
            </Col>
            <Col>
              <AppButton className='web-info__button' text='Lưu' variant='primary' onClick={handleClickSave} />
            </Col>
          </>
        )}
      </Row>
