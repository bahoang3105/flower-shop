...
export default function DetailFlower() {
  ...
  const [displayListThumbnail, setDisplayListThumbnail] =
  ...
  const handleSubmit = async () => {
    ...
    const topicsDel = topicIds?.filter(
      (id: number) => !updateTopicIds?.includes(id)
    );
    const topicsAdd = updateTopicIds?.filter(
      (id: number) => !topicIds?.includes(id)
    );
    const listImageUploaded = listImage
    ...
    topicsDel.forEach((topicId: number) =>
      formData.append("topicsDel", String(topicId))
    );
    topicsAdd.forEach((topicId: number) =>
      formData.append("topicsAdd", String(topicId))
    );
    Object.keys(flowerInfo).forEach((field: string) =>
      formData.append(field, flowerInfo[field])
    );
    listImageUploaded?.forEach((imageSrc: string) =>
      formData.append("listImage", imageSrc)
    );
  ...
  return (
    <div className="flower-detail">
      ...
      <Row gutter={40}>
        <Col span={8}>
          <Row
            className="flower-detail__preview"
            onMouseEnter={showListThumbnail}
            onMouseLeave={hideListThumbnail}
          >
            {listImage[curThumbnail] && (
              <ImageAntd src={listImage[curThumbnail].url} preview={false} />
            )}
            <div
            ...
            >
              <Slider
                curSlide={curThumbnail}
                setCurSlide={setCurThumbnail}
                lastSlide={listImage.length - 1}
              >
                {renderListImage()}
              </Slider>
            </div>
          ...
          <Row className="flower-detail__update-thumbnail">
            <Collapse
              collapsible={isEdit ? "header" : "disabled"}
              activeKey={isEdit ? ["1"] : []}
            >
              <Collapse.Panel
                header={
                  <Tooltip
                    title={
                      isEdit
                        ? ""
                        : "Nhấn chỉnh sửa để cập nhật danh sách ảnh hoa"
                    }
                  >
      ...
      >
        <h2>
          Hành động này sẽ không thể hoàn tác. Bạn có chắc chắn xóa&nbsp;
          <strong>{flowerInfo.name}</strong>
          &nbsp;hay không?
        </h2>
        <Row
